...
async function main() {
  ...
  try {
    ...
    responder = () => ({
      status: "success",
      pong: true,
      bridgeVersion: EXPECTED_VERSION,
      aeVersion: "FAKE-26",
      bridgeFolder: DIR,
      project: "P.aep",
      activeComp: null,
    ...
    expect(
      "A: check-bridge ok:true & versionMatch",
      a.parsed && a.parsed.ok === true && a.parsed.versionMatch === true,
    );
    ...
    const b = await client.callTool("execute-script", {
      script: 'throw new Error("x")',
      timeoutMs: 6000,
    });
    log("   isError=", b.isError, " text=", b.text.replace(/\s+/g, " ").slice(0, 120));
    expect(
      'B: AE error matched within timeout (not "Timed out")',
      b.parsed && !/Timed out/.test(b.text),
    );
    ...
    const oneOk =
      (c1.parsed && c1.parsed.echo === "MARKER_ONE") ||
      (c2.parsed && c2.parsed.echo === "MARKER_ONE");
    const twoOk =
      (c1.parsed && c1.parsed.echo === "MARKER_TWO") ||
      (c2.parsed && c2.parsed.echo === "MARKER_TWO");
    ...
    expect(
      "C: both concurrent calls returned their OWN result (no clobber)",
      oneOk && twoOk && distinct,
    );
    ...
    expect(
      'D: stalePanelDetected:true (not just "no response")',
      d.parsed && d.parsed.stalePanelDetected === true,
    );
    expect(
      "D: reports the version the stale panel claimed",
      d.parsed && d.parsed.panelReportedVersion === EXPECTED_VERSION,
    );
  } finally {
    stopAE();
    try {
