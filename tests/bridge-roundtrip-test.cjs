...
function startFakeAE(versionToReturn, label) {
  ...
  const timer = setInterval(() => {
    try {
      ...
      if (cmd.commandId && !seen.has(cmd.commandId)) {
        seen.add(cmd.commandId);
        log(`   [fakeAE:${label}] SAW cmd ${JSON.stringify(cmd.commandId)}`);
      }
      ...
      log(
        `   [fakeAE:${label}] answered ping commandId=${JSON.stringify(cmd.commandId)} -> wrote _commandId=${JSON.stringify(result._commandId)} to ${RES_FILE}`,
      );
    } catch {
...
async function main() {
  ...
  // Clean slate
  for (const f of [CMD_FILE, RES_FILE]) {
    try {
      fs.unlinkSync(f);
    } catch {}
  }
  ...
  if (!fs.existsSync(serverPath)) {
    console.error("MISSING build/index.js - run npm run build");
    process.exit(2);
  }
  ...
  child.stderr.on("data", () => {
    /* server logs to stderr; swallow for clean output */
  });
  ...
  try {
    ...
    log(
      `[tools] ${names.length} tools registered; check-bridge present: ${names.includes("check-bridge")}`,
    );
    ...
    const pingSeen =
      fs.existsSync(CMD_FILE) && JSON.parse(fs.readFileSync(CMD_FILE, "utf8")).command === "ping";
    ...
    log(
      "   [debug] final RES_FILE after B:",
      fs.existsSync(RES_FILE) ? fs.readFileSync(RES_FILE, "utf8") : "(none)",
    );
    log(
      "   [debug] final CMD_FILE after B:",
      fs.existsSync(CMD_FILE) ? fs.readFileSync(CMD_FILE, "utf8") : "(none)",
    );
    ...
    log(
      "   [debug] final RES_FILE after C:",
      fs.existsSync(RES_FILE) ? fs.readFileSync(RES_FILE, "utf8") : "(none)",
    );
    log(
      "   [debug] final CMD_FILE after C:",
      fs.existsSync(CMD_FILE) ? fs.readFileSync(CMD_FILE, "utf8") : "(none)",
    );
  } finally {
    try {
  ...
  const checks = [];
  checks.push([
    "A: no-AE reports not-responding",
    results.A && results.A.ok === false && /No response/i.test(results.A.problem || ""),
  ]);
  checks.push([
    "A: exposes expectedBridgeVersion",
    results.A && results.A.expectedBridgeVersion === EXPECTED_VERSION,
  ]);
  checks.push([
    "B: bridge responding ok",
    results.B && results.B.ok === true && results.B.bridgeResponding === true,
  ]);
  checks.push(["B: version match true", results.B && results.B.versionMatch === true]);
  checks.push([
    "B: reports AE version & project",
    results.B &&
      /SIMULATED/.test(results.B.aeVersion || "") &&
      /SIMULATED/.test(results.B.project || ""),
  ]);
  checks.push([
    "C: responding but versionMatch false",
    results.C && results.C.ok === true && results.C.versionMatch === false,
  ]);
