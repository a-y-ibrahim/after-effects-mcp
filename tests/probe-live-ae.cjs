...
function readExpectedVersion() {
  ...
  try {
    ...
    if (m) return m[1];
    console.warn('WARN: EXPECTED_BRIDGE_VERSION not found in', serverPath, '- version match label may be wrong');
  } catch {
    console.warn('WARN: could not read', serverPath, '(run npm run build) - version match label may be wrong');
  }
  return null;
}
...
async function main() {
  ...
  fs.writeFileSync(RES_FILE, JSON.stringify({ status: 'waiting', message: 'probe waiting', timestamp: new Date().toISOString() }, null, 2), 'utf8');

  // write the ping command exactly as the server does
  const cmd = { command: 'ping', args: {}, commandId: id, timestamp: new Date().toISOString(), status: 'pending' };
  ...
  while (Date.now() - start < 10000) {
    try {
      if (fs.existsSync(RES_FILE)) {
        ...
        if (raw && raw.trim()) {
          let parsed = null;
          try { parsed = JSON.parse(raw); } catch { /* mid-write */ }
          if (parsed && parsed.status !== 'waiting') {
            if (raw !== lastShown) { lastShown = raw; }
            ...
            console.log('bridgeVersion:      ', JSON.stringify(parsed.bridgeVersion), EXPECTED_VERSION == null ? '(server EXPECTED version unknown - no build)' : parsed.bridgeVersion === EXPECTED_VERSION ? '(matches server EXPECTED)' : `(server EXPECTS ${EXPECTED_VERSION})`);
            ...
            console.log('echoes _commandId:  ', parsed._commandId !== undefined, parsed._commandId !== undefined ? `(=${JSON.stringify(parsed._commandId)}, ${parsed._commandId === id ? 'MATCHES our id' : 'does NOT match'})` : '<-- MISSING: check-bridge id-match will FAIL');
            console.log('echoes _commandExecuted:', parsed._commandExecuted !== undefined, parsed._commandExecuted !== undefined ? `(=${JSON.stringify(parsed._commandExecuted)})` : '<-- MISSING: check-bridge fallback-match will FAIL too');
            ...
            const fallbackMatch = parsed._commandId === undefined && parsed.status !== 'waiting' && parsed._commandExecuted === 'ping';
            console.log('\nVERDICT: live AE panel is', parsed.pong ? 'RESPONDING' : 'present but no pong', '|',
              (idMatch || fallbackMatch) ? 'check-bridge WOULD MATCH this result -> bridge healthy' : 'check-bridge would NOT match -> stale panel, needs reinstall/reopen');
            process.exit(0);
          }
        }
      }
  ...
  console.log('=> Either AE is not running, the panel is closed, or "Allow Scripts to Write Files" is off.');
  console.log('Final result file:', fs.existsSync(RES_FILE) ? fs.readFileSync(RES_FILE, 'utf8') : '(none)');
  console.log('Final command file status:', fs.existsSync(CMD_FILE) ? JSON.parse(fs.readFileSync(CMD_FILE, 'utf8')).status : '(none)');
  process.exit(1);
}
